...
const jsPaths = [
  ...
  },
  {
    ...
    src: ["src/assets/js/app.js", "src/assets/js/components/**/*.js"],
  },
  {
...
gulp.task("styles", async function () {
  return stylePaths
    .reduce(function (streams, current_value) {
      const sassFilter = filter("**/*.{scss,sass}", { restore: true });
      return streams.add(
        gulp
          .src(current_value.src, { allowEmpty: true })
...
gulp.task(
  ...
  gulp.series(
    ...
    function () {
      ...
      gulp
        .watch(
          assetsPaths.reduce((acc, val) => acc.concat(val.src), []),
          gulp.series("assets")
        )
        .on("change", server.reload);
      gulp
